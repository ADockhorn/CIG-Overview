...
\title{Computational Intelligence in Games\\ - Quick Reference -}
...
\begin{definition}[Strategy profile]
	A strategy profile is the set of strategies taken by agents to play in one game. Assume a strategy profile with $n$ strategies $S = (s_1, \dots, s_n)$. Each player gets one of them.
	\vspace{1em}
	All strategies except $s_i$ are denoted by $s_{-i} = (s_1, \dots, s_{i-1}, s_{i+1}, \dots, s_n)$
\end{definition}
\vspace{1em}

\begin{definition}[Best Response:]  $s_i$ is a best response to $s_{-i}$ if and only if
	$$\pi_{s_i, s_{-i}} \geq \pi_{s', s_{-i}} \quad \forall s' \in S$$
\end{definition}	
\vspace{1em}

\begin{definition}[Nash Equilibrium:] The strategy profile $S = (s_1, \dots, s_n)$ is a Nash Equilibrium if and only if for each player $i$, his strategy $s_i$ is a best response to $s_{-i}$\\
\end{definition}
...
\begin{algorithm}[H]
	...
	\KwOut{$V \approx v_\pi$}    
	\caption{Monte Carlo Method $v(s)$}
\end{algorithm} 
...
\begin{algorithm}[H]
	...
	\KwOut{$V \approx v_\pi$}    
	\caption{Constant-$\alpha$ Monte Carlo Method $v(s)$}
\end{algorithm} 
...
\subsection{Flat Upper Confidence Bounds (UCB)}
	...
	\begin{algorithm}[H]
		...
		Initialize an empty list of \textit{Returns($s_t,a$)} $\forall a \in A$\;
		\;
		\For{$k$ iterations}{
			Select first action $a$ of the simulation using UCB\;
			Simulate episode using random policy\;
			Update visit counts $N(s_t) \leftarrow k\quad$ and  $\quad N(s_t, a) \leftarrow N(s_t, a) + 1$\;
			Returns($s_t, a$).append(return of episode k)\;
			$Q(s_t,a) \leftarrow \textit{mean}(\textit{Returns}(s_t, a))$;
		}
		\KwOut{$\argmax_a Q(s_t,a)$}    
		\caption{Flat UCB}
	\end{algorithm}
...
\subsection{NSGA-II Algorithm}
	...
	\begin{algorithm}[H]
		\DontPrintSemicolon
		\KwIn{A single front $F$ with $N$ individuals}
		Initialize distances for all individuals: $d_i = 0, \quad \forall i \in F$\;
		\vskip0.5em
		\ForEach{each objective $m$}{
			Sort individuals in $F$ regarding their objective values $f_m(i)$\;
			Assign $d_i \leftarrow \infty$ to the elements with index 1 and $N$ in the \mbox{sorted list} $\lbrack$the elements with lowest or highest value in $f_m(i)\rbrack$ \;
			\vskip0.5em
			$f_m^{min}, f_m^{max} \leftarrow$ minimum and maximum observed value for objective value $f_m(i)$ for individuals in $F$ \;
			\vskip0.5em
			\For{all individuals $i$ with index $i_{idx}=2$ to $N-1$}{
				$d_i \leftarrow d_i + \big\vert\frac{f_m(i_{idx+1}) - f_m(i_{idx-1})}{f_m^{max} - f_m^{min}}\big\vert$\;
			}
		}
		\KwOut{Crowding Distance $d(i), \quad \forall i \in F$}    
...
\vskip2em
	...
	\begin{algorithm}[H]
		...
		Fronts $(F_1, F_2, \cdots) \leftarrow$ non-dominated sorting of $P(t) \cup M''(t)$\;
		Set $P(t+1) = \emptyset, \quad i=1, \quad N = \vert P(t)\vert$\;
		\vskip1.0em
		\Repeat{$\vert P(t+1)\vert + \vert F_i\vert > N$}{
			$P(t+1) \leftarrow P(t+1)\cup F_i$\;
			$i \leftarrow i+1$\;
		}
		\vskip1.0em
		Sort all individuals in $F_i$ by their crowding distance\;
		$P(t+1) \leftarrow P(t+1)~\cup $ $\lbrace$first $N-\vert P(t+1)\vert$ elements in $F_i\rbrace$\;
		\vskip1.0em
